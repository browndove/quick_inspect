...
import { FacilityTypePicker } from '@/components/forms/facility-type-picker';
import { FormPickerField } from '@/components/forms/form-picker-field';
import { FormTextField } from '@/components/forms/form-text-field';
import { MmdaDistrictPicker } from '@/components/forms/mmda-district-picker';
...
import type { PharmacyRoutinePart1Draft } from '@/types/pharmacy-routine';

export type { PharmacyRoutinePart1Draft } from '@/types/pharmacy-routine';
...
const emptyPart1 = (): PharmacyRoutinePart1Draft => ({
  facilityName: '',
  region: '',
  mmda: '',
  facilityType: null,
...
export default function PharmacyRoutinePart1Screen() {
  const insets = useSafeAreaInsets();
  const { selectedRegion } = useSelectedRegion();
  const [form, setForm] = useState<PharmacyRoutinePart1Draft>(emptyPart1);
  ...
  useEffect(() => {
    const key =
      resolveGhanaRegionKey(form.region) ??
      resolveGhanaRegionKey(selectedRegion ?? null);
    if (!key) {
      setForm((f) => (f.mmda ? { ...f, mmda: '' } : f));
      return;
    }
    const allowed = new Set(GH_MMDA_BY_REGION[key]);
    setForm((f) => {
      if (!f.mmda || allowed.has(f.mmda)) return f;
      return { ...f, mmda: '' };
    });
  }, [form.region, selectedRegion]);
        ...
        <KeyboardAvoidingView
          ...
          <ScrollView
            ...
            <RoutineIntroCard
              step={PART_INDEX}
              total={PART_TOTAL}
              description="Facility details and compliance for this pharmacy routine visit."
            />
            ...
            <IosFieldCard>
              <MmdaDistrictPicker
                region={form.region}
                selectedRegionHint={selectedRegion}
                value={form.mmda}
                onChange={(mmda) => setForm((f) => ({ ...f, mmda }))}
              />
            </IosFieldCard>
            <IosFieldCard>
              <FacilityTypePicker
                value={form.facilityType}
                onChange={(t) => setForm((f) => ({ ...f, facilityType: t }))}
              />
            </IosFieldCard>
            ...
            <IosFieldCard>
              <FormPickerField
                label="Number of visit within a year"
                ...
                onChange={(v) =>
                  setForm((f) => ({
                    ...f,
                    visitOrder: v as PharmacyRoutinePart1Draft['visitOrder'],
                  }))
                }
          ...
          <View
            ...
            <Pressable
              onPress={() => {
                ...
                router.push('/inspection/routine/pharmacy/part2');
              }}
              style={({ pressed }) => [
                styles.cta,
