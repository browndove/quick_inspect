...
from app.schema_bootstrap import CORE_PUBLIC_TABLE_NAMES, missing_core_public_tables
...
async def health() -> dict:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            missing = await missing_core_public_tables(conn)
            base["database"]["missingCoreTables"] = missing
            base["database"]["coreTablesPresent"] = len(CORE_PUBLIC_TABLE_NAMES) - len(missing)
            ...
            if base["database"]["coreTablesPresent"] < base["database"]["coreTablesExpected"]:
                base["database"]["hint"] = (
                    f"Incomplete schema (missing: {', '.join(missing)}). "
                    "Migrations run in the background on boot; wait ~30s and refresh, or redeploy."
                )
